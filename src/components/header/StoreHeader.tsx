...
const StoreHeader: React.FC = () => {
  ...
  return (
    ...
    >
      <div
        className={`ap-flex ap-gap-2 ${display.mdAndDown ? "ap-justify-between ap-w-full " : "ap-h-8 ap-items-center"}`}
      >
        <img
          onClick={() => navigate("/inicio")}
          src={!location.pathname.startsWith("/clase") ? logoSVG : logoSVGLight}
          alt="logo"
      ...
      {!display.mdAndDown && (
        <ul className="ap-list-none ap-text-white ap-flex ap-gap-5 !ap-text-sm">
          {headers.map((x, index) => (
            <li key={index}>
              <button
                onClick={() => {
      ...
      <div className="ap-flex ap-gap-2">
        ...
        {/* {!display.mdAndDown && (
          <Link
            to={`/cursos`}
            className=" ap-block ap-text-center ap-bg-[#FFC62D] ap-text-black ap-text-sm ap-rounded-lg ap-py-2 ap-px-4"
          >
            {!display.mdAndDown && "Cursos e"} Inscripciones
          </Link>
        )} */}
      </div>
      ...
      >
          ...
          (header, index) =>
            header.segment && (
              <MenuItem
                ...
                onClick={() => {
                  if (header.route) {
                    navigate(header.route);
                    handleCloseMenu();
                    return;
                  }
                  scrollToSection(header.segment);
                }}
        ...
        >
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className=" ap-block ap-w-full ap-text-center ap-border ap-border-[#FFC62D]  ap-text-[#FFC62D] ap-text-sm ap-rounded-lg ap-py-2 ap-px-4"
          >
            Aula virtual
          </a>
        </MenuItem>
        ...
        >
          <Link
            to={`/cursos`}
            className=" ap-block ap-w-full ap-text-center ap-bg-[#FFC62D]  ap-text-black ap-text-sm ap-rounded-lg ap-py-2 ap-px-4"
          >
            {!display.mdAndDown && "Cursos e"} Inscripciones
          </Link>
        </MenuItem> */}
